...
import { MagaryComponentDocs } from './component-scanner.js';
...
class MagaryMcpServer {
  ...
  private components: MagaryComponentDocs[] = [];
  ...
  async init() {
    ...
    try {
      ...
      console.error(
        `Magary MCP: Loaded ${this.components.length} components from catalog.`,
      );
    } catch (e) {
      console.error(
        `Magary MCP ERROR: Could not load catalog from ${catalogPath}. Ensure you run 'pnpm run build' first.`,
      );
      process.exit(1);
    }
