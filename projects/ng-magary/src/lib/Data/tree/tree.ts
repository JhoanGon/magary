...
export class MagaryTree {
  ...
  filterTree(nodes: MagaryTreeNode[], term: string): MagaryTreeNode[] {
    ...
    for (const node of nodes) {
      ...
      if (matches || childMatches) {
        if (childMatches) copy.expanded = true;
        filtered.push(copy);
      }
    }
  ...
  isNodeMatching(node: MagaryTreeNode, term: string): boolean {
    ...
    if (this.filterMode() === 'strict') return label === term;
  ...
  handleDrop(event: CdkDragDrop<MagaryTreeNode[]>, parent: MagaryTreeNode | null) {
    ...
    if (this.validateDrop() && !this.isDropAllowed(dropEvent)) return;
    this.onNodeDrop.emit(dropEvent);
  }

  handleNodeDrop(event: MagaryTreeNodeDropEvent) {
    if (this.validateDrop() && !this.isDropAllowed(event)) return;
  ...
  private isDropAllowed(event: MagaryTreeNodeDropEvent): boolean {
    ...
    if (this.isSameNode(dragNode, targetParent)) return false;
  ...
  private containsNode(root: MagaryTreeNode, target: MagaryTreeNode): boolean {
    ...
    for (const child of root.children) {
      if (this.isSameNode(child, target) || this.containsNode(child, target)) return true;
    }
    return false;
  }
